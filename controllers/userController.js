...
//       res.status(400).json({ message: "User followed succesfully" });
//     } else {
//       //follow user
//       // modify current user following,modify followers of userToModify
//       await User.findByIdAndUpdate(req.user._id, { $push: { following: id } });
//       await User.findByIdAndUpdate(id, { $push: { following: req.user._id } });
//       res.status(400).json({ message: "User unfollowed succesfully" });
//     }
//   } catch (err) {
//     res.status(500).json({
//       message: err.message,
...
const updateUser = async (req, res) => {
  const { name, email, username, profilePic, bio, password } = req.body;
  ...
  try {
    ...
    console.log("Error in updatUser:", err.message);
  }
};
